...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--outcomes", type=int, default=3)
    s.add_argument("--close-in", type=int, default=600, help="Seconds until close")
    s.add_argument("--settle-after", type=int, default=1800, help="Seconds after close to settle deadline")
    s.add_argument("--min-stake", type=float, default=25.0)
    s.add_argument("--max-stake", type=float, default=1500.0)
    s.add_argument("--max-orders", type=int, default=90)
    s.add_argument("--allow-unmatched", action="store_true")
    s.add_argument("--fee-bps", type=int, default=240)
    s.add_argument("--rebate-bps", type=int, default=110)
    s.set_defaults(fn=cmd_market_create)

    s = sub.add_parser("market-list", help="List markets")
    s.set_defaults(fn=cmd_market_list)

    s = sub.add_parser("order-post", help="Post a quote/order")
    s.add_argument("market_id", type=int)
    s.add_argument("--maker", required=True)
    s.add_argument("--side", required=True, choices=["BACK", "LAY", "back", "lay"])
    s.add_argument("--outcome", type=int, required=True)
    s.add_argument("--price-e4", type=int, required=True, help="Odds * 1e4, e.g. 18600 for 1.8600")
    s.add_argument("--size", type=float, required=True)
    s.add_argument("--expiry-in", type=int, default=420, help="Seconds until expiry")
    s.set_defaults(fn=cmd_order_post)

    s = sub.add_parser("order-cancel", help="Cancel an order")
    s.add_argument("order_id")
    s.add_argument("--by", required=True)
    s.set_defaults(fn=cmd_order_cancel)

    s = sub.add_parser("order-take", help="Take/Fill an order")
    s.add_argument("order_id")
    s.add_argument("--taker", required=True)
    s.add_argument("--stake", type=float, required=True)
    s.set_defaults(fn=cmd_order_take)

    s = sub.add_parser("settle", help="Settle market outcome")
    s.add_argument("market_id", type=int)
    s.add_argument("outcome", type=int)
    s.set_defaults(fn=cmd_settle)

    s = sub.add_parser("void", help="Void market")
    s.add_argument("market_id", type=int)
    s.set_defaults(fn=cmd_void)

    s = sub.add_parser("claim", help="Claim payout for a match")
